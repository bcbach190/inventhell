...
func on_buy_problem(problem_type: String):
	...
	if purchase_result.success:
		...
		if problem_type in ["Calculator", "Display Case", "Upgrade Medal"]:
			...
			# For now, just print that a tool was bought. We can add it to a separate tool inventory later.
			print("Player bought a tool: %s" % problem_type)
			return {"success": true, "is_tool": true}
		...
		if problem_generator:
			...
			# Create an "unrevealed" item
			var unrevealed_item = {
				"revealed": false,
				"problem_data": problem_data
			}
			
			# Add the unrevealed item to the player's inventory
			if currency_manager:
				currency_manager.add_to_inventory(unrevealed_item)
			...
			return {"success": true, "item": unrevealed_item, "is_tool": false}
		else:
