...
# Whiteboard slot
# NOTE: Make sure to add a Panel node named 'WhiteboardSlot' to the scene.
@onready var whiteboard_slot: Panel = $WhiteboardSlot
...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	return data is Dictionary and data.has("revealed")

func _drop_data(at_position: Vector2, data: Variant) -> void:
	if current_problem == null:
		current_problem = data
		GlobalCurrency.instance.remove_from_inventory(data)
		_display_problem()

func _display_problem():
	for child in whiteboard_slot.get_children():
		child.queue_free()

	var problem_text = current_problem.problem_data.problem
	var problem_label = Label.new()
	problem_label.text = problem_text
	whiteboard_slot.add_child(problem_label)

	var answer_edit = LineEdit.new()
	answer_edit.name = "AnswerEdit"
	whiteboard_slot.add_child(answer_edit)

	var submit_button = Button.new()
	submit_button.text = "Submit"
	submit_button.pressed.connect(_on_submit_answer)
	whiteboard_slot.add_child(submit_button)

func _on_submit_answer():
	var answer_edit = whiteboard_slot.get_node("AnswerEdit")
	var user_answer = answer_edit.text

	if str(user_answer) == str(current_problem.problem_data.answer):
		GlobalCurrency.instance.add_currency(10, "Solved problem")
		current_problem = null
		for child in whiteboard_slot.get_children():
			child.queue_free()
		var correct_label = Label.new()
		correct_label.text = "Correct!"
		whiteboard_slot.add_child(correct_label)
	else:
		var incorrect_label = Label.new()
		incorrect_label.text = "Incorrect!"
		whiteboard_slot.add_child(incorrect_label)
		var timer = get_tree().create_timer(1.0)
		timer.timeout.connect(func(): incorrect_label.queue_free())
